...
public interface Player {
   ...
   * ，illegalAction为null。
   *
   * @param contextView 游戏上下文
   * @param actionTypes 可选动作类型
   * @return 要执行的动作
   * @throws InterruptedException 线程被中断时抛出此异常。选择过程中随时可能被中断，实现时应该经常检查。
   */
   ...
   * @param contextView   游戏上下文
   * @param actionTypes   可选动作类型
   * @param illegalAction 如果非null，则表示上一次选择的动作不符合规则，需要重新选择。此参数提供上次选择的动作。
   * @return 要执行的动作
   * @throws InterruptedException 线程被中断时抛出此异常。选择过程中随时可能被中断，实现时应该经常检查。
   */
   ...
   *
   * @param secondsToGo 剩余秒数。null表示倒计时结束或取消。
   */
