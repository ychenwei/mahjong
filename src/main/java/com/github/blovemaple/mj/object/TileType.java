...
public class TileType implements Serializable, Comparable<TileType> {
  ...
  private static TileRank getRank(int i) {
    switch (i) {
      case EAST:
        return TileRank.ZiRank.DONG_FENG;
      case SOUTH:
        return TileRank.ZiRank.NAN;
      case WEST:
        return TileRank.ZiRank.XI;
      case NORTH:
        return TileRank.ZiRank.BEI;
      case RED:
        return TileRank.ZiRank.ZHONG;
      case GREEN:
        return TileRank.ZiRank.FA;
      case WHITE:
        return TileRank.ZiRank.BAI;
    }
   ...
   * Range 1-9
   *
   * @throws UnsupportedOperationException 种类不是数字
   */
  public int number() {
    if (!isNumberRank())
